...
| **8443** | Surface **xr-spark-hub-proxy** (optional; forwards to `https://<DGX_LAN_IP>:8088`) |
...
DGX hub only (no Surface proxy — headset will get **connection refused** on `<SURFACE_LAN_IP>:8443`):

```bash
bash /home/sifr/3DAIGC-API/mcp/scripts/run_xr_ai_3daigc_stack.sh
```

**Galaxy XR URL:** `https://<SURFACE_LAN_IP>:8443` (Surface proxy → DGX `:8088`). Not bare `<SURFACE_LAN_IP>`, not `http://`.
...
Open hub UI on DGX LAN: `https://<DGX_LAN_IP>:8088` (accept self-signed cert). Start mic; try *“make a 3D model of this”*.

## Galaxy XR + router client isolation

Some routers block **headset → DGX** (`<HEADSET_LAN_IP>` → `<DGX_LAN_IP>`) but allow **headset → Surface** (`<SURFACE_LAN_IP>`).

**On Surface** (uses `certs/localhost.pem` from `npm run setup-https`):

```powershell
cd <OPENNEXUS_REPO_ROOT>
$env:XR_SPARK_HUB_URL = 'https://<DGX_LAN_IP>:8088'
