# State — rewritten in full at session END. Cap: 40 lines.

Session: 16
Focus: Env-scan lock-in pushed
Active: API f0182c6 + OpenNexus c0f88c3d on main
Next: restart API workers when idle; hosted billing tiers later
Blocked: none

## Watch-outs
- prefer_floor + PCA flatten + wall-vs-camera; Phase B densify OFF
- Door metric 0.762 reference_length horizontal
- Creature/retopo WIP still uncommitted in working trees

## Recently shipped
- API wall-fallback gravity + PCA flatten + quat sanitize (f0182c6)
- OpenNexus Phase A/B UI docs + LingBot splat seating (c0f88c3d)
- Docs v3.3.7 env-scan Phase A/B (2026-07-30)
