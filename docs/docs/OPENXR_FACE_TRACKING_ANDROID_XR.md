...
**Dev topology:** Vite and `logs/remote-log.txt` run on the **Surface Laptop**; the **DGX Spark** is headless remote via **NVIDIA Sync** / SSH. See [`DEV_MACHINE_TOPOLOGY.md`](./DEV_MACHINE_TOPOLOGY.md).
...
When the browser does not grant **`expression-tracking`**, use the **OpenNexus XR Face** APK plus the Vite dev relay on the **main app** (`/`), not the IWSDK lab (`/xr`):

| Step | Component |
|------|-----------|
| 1 | `npm run dev` on PC — enables `POST /__native_face_ingest` and `GET /__native_face_sse` |
| 2 | APK **Jetpack XR** → HTTP POST to ingest (~30 Hz). **`FaceTrackingCoordinator.OPENXR_ENABLED` is `false`** today (OpenXR reserved for a future runtime). **FaceKeeper** + PiP keep Jetpack alive during Chrome **Full Space** |
| 3 | Chrome opens `?nativeFaceRelay=1` → [`nativeFaceRelay.js`](../src/library/nativeFaceRelay.js) EventSource → `nativeFaceBridge` |
| 4 | XR frame loop in [`sceneManager.js`](../src/library/sceneManager.js) uses native weights (same as WebView); web cache **30s** while `xrPresenting`, APK handoff stale **10s**. Dev: [`NativeFaceRelayHud.jsx`](../src/components/NativeFaceRelayHud.jsx) when `?nativeFaceRelay=1` |
