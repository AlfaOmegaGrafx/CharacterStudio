...
**Galaxy XR workaround (dev):** Use the **main app** at `https://<dev-workstation-LAN-IP>:3000/` (not `/xr`) — typically your **Surface Laptop** on the LAN, not the DGX Spark. Install the [**CS XR Face** APK](../native/android-xr-face-bridge/README.md), run **`npm run dev`** on that workstation, open **⋮ → Open in Chrome for WebXR (+ face)** (`?nativeFaceRelay=1` + `?remoteLog=1`), and **keep the APK visible or in PiP** so **Jetpack XR** can relay to the dev server during Chrome **Full Space** AR (see [OpenXR face tracking](./OPENXR_FACE_TRACKING_ANDROID_XR.md), [Android Studio AI brief](./ANDROID_STUDIO_AI_BRIEF.md), and [dev machine topology](./DEV_MACHINE_TOPOLOGY.md)). OpenXR face is **disabled** in current APK builds (`OPENXR_ENABLED=false`).
