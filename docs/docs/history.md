...
- **Immersive XR (in repo)**: Main app **WebXR** via `SceneManager` (VR/AR, floor anchoring); **IWSDK lab** at `/xr` for locomotion/grab experiments ([`docs/docs/IWSDK_INTEGRATION.md`](./IWSDK_INTEGRATION.md)); **Galaxy XR** + optional [**OpenNexus XR Face** APK](../../native/android-xr-face-bridge/README.md) for blend-shape relay when Chrome lacks expression-tracking ([`docs/docs/OPENXR_FACE_TRACKING_ANDROID_XR.md`](./OPENXR_FACE_TRACKING_ANDROID_XR.md))
...
- Integrate with external 3D launchpads (e.g. Solana/Arweave‑based VRM/GLB minting) so avatars and wearables minted there can be assembled from wallet‑owned assets inside OpenNexus3DStudio
