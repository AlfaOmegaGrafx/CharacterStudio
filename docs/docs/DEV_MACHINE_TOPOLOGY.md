...
|------|----------|----------------|---------------------------|
| **Dev workstation** | Windows **Surface Laptop Studio 2** | Local Cursor, Chrome/Edge, `npm run dev`, webcam tests | **`<SURFACE_LAN_IP>`** (example LAN IP when Vite binds on the Surface) |
...
| **XR headset** | **Galaxy XR** (Chrome WebXR) | Opens `https://<dev-workstation-LAN-IP>:3000/...` on the LAN | **`<HEADSET_LAN_IP>`** (example) |
...
```text
[REMOTE_LOG][::ffff:<HEADSET_LAN_IP>][session=…][info] … (https://<SURFACE_LAN_IP>:3000/?…)
```

| Field | Meaning |
|-------|---------|
| `[::ffff:<HEADSET_LAN_IP>]` | **Who sent** the log (e.g. Galaxy XR Chrome) |
| `(https://<SURFACE_LAN_IP>:3000/…)` | **Dev server origin** the tab was loaded from (Surface Vite) |
...
|------|----------------------|
| `webcamDebug=1` | **Surface** browser (`<SURFACE_LAN_IP>`) |
| `nativeFaceRelay=1` | **Headset** Chrome (`<HEADSET_LAN_IP>`); ingest handled by Surface Vite |
...
- **SSH (two hosts only):** **`DGX-Local`** = LAN `<DGX_LAN_IP>`; **`DGX-Remote`** = Tailscale via NVIDIA Sync. User `sifr`. See [`SSH_HOST_NAMES.md`](./SSH_HOST_NAMES.md).
...
|-----------|--------------|-------------------|
| **Surface (local dev + Galaxy XR)** | `<LEGACY_CHARACTERSTUDIO_ROOT>` | Local folder |
...
```env
DEV_API_PROXY_TARGET=http://<DGX_LAN_IP>:7842
...
| Route | SSH host | When |
|-------|----------|------|
| Same Wi‑Fi | **`DGX-Local`** | Spark at `<DGX_LAN_IP>` |
...
**Critical:** If you browse **`https://<SURFACE_LAN_IP>:3000`**, that is **Surface `npm run dev`**. Pushing files to DGX does **not** change that browser session. DGX sync is for the **Remote-SSH Cursor agent** on the Spark and for sharing docs with the **3DAIGC-API** codebase — not for updating the Surface dev server.
