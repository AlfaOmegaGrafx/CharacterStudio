...
**Dev topology:** Vite and `logs/remote-log.txt` run on the **Surface Laptop** (LAN IP e.g. `<SURFACE_LAN_IP>`). The **DGX Spark** is reached headless via **NVIDIA Sync** / SSH (`sifr`), not the desktop browser for webcam tests. The **Galaxy XR** headset is typically `<HEADSET_LAN_IP>` in logs. See [`DEV_MACHINE_TOPOLOGY.md`](./DEV_MACHINE_TOPOLOGY.md).
...
**Current runtime (May 2026):** `FaceTrackingCoordinator.OPENXR_ENABLED = false` — **Jetpack XR only**. OpenXR `XR_ANDROID_face_tracking` session creation fails on Galaxy XR (-2 / -38); the native OpenXR code stays in the repo behind the flag. Web mirror: `OPENXR_WEB_ENABLED = false` in `nativeFaceBridge.js`.

**Solution:** A companion APK (`com.opennexus3dstudio.xrfacebridge`, folder `native/android-xr-face-bridge/`) that:

1. Runs **native face tracking** (Jetpack XR; OpenXR when the flag is re-enabled).
...
[Galaxy XR headset]
  OpenNexus XR Face APK
    ...
    FaceHttpRelay → POST https://<PC_LAN_IP>:3000/__native_face_ingest
                                             │
[Dev workstation — Surface] npm run dev (Vite) │
...
- Use the **main OpenNexus3DStudio URL** (`https://<PC_IP>:3000/`), **not** `/xr` (IWSDK lab has no VRM / face relay UI).
- In Chrome AR: **`nativeKeys > 25`**, **`relay=poll+sse/<500ms`**, sustained while **`xrPresenting=true`** (live ingest, not frozen cache). Dev HUD: green **Face relay** overlay when `?nativeFaceRelay=1`.
- Logcat: `ON-JetpackFace` `TRACKING` + `ON-FaceHttpRelay` posts. (`ON-OpenXrFace` only if `OPENXR_ENABLED` is flipped back to `true`.)
...
1. Surface (dev workstation): `npm run dev` → `https://<Surface-LAN-IP>:3000/?remoteLog=1&nativeFaceRelay=1` (see [`DEV_MACHINE_TOPOLOGY.md`](./DEV_MACHINE_TOPOLOGY.md))
