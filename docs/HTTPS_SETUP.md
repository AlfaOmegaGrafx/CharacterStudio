...
3. **Generate certificates:**
   ```bash
   mkcert localhost 127.0.0.1 ::1 <SURFACE_LAN_IP>
   ```
...
6. **Access via HTTPS:**
   ...
   - `https://<SURFACE_LAN_IP>:3000` (for Galaxy XR device)
...
2. Find your computer's IP address (e.g., `<SURFACE_LAN_IP>`)
3. Add the IP to the certificate (see mkcert step 3 above)
4. Access via `https://<SURFACE_LAN_IP>:3000` on the Galaxy XR device
