...
- `VITE_XR_HUB_URL=https://dgx-spark.<TAILNET_ID>.ts.net:8088` — **XR Voice sidebar** (Tailscale Funnel to DGX Spark hub). Public HTTPS URL only — **not** a secret. Do **not** put `MSF_EDIT_KEY`, API keys, or LAN IPs here.
...
VERCEL=1 CI=1 VITE_PUBLIC_DEMO=1 \
  ...
  VITE_XR_HUB_URL=https://dgx-spark.<TAILNET_ID>.ts.net:8088 \
  npm run build
```
