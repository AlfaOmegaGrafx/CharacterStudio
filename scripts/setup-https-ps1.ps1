...
if ($opensslFound) {
    ...
    $opensslCommand = "`"$opensslFound`" req -x509 -newkey rsa:2048 -keyout `"$keyPath`" -out `"$certPath`" -days 365 -nodes -subj `/C=US/ST=State/L=City/O=Organization/CN=localhost`" -addext `"subjectAltName=DNS:localhost,DNS:127.0.0.1,IP:<SURFACE_LAN_IP>`""
